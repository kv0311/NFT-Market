...
contract SVGNFT is ERC721URIStorage, Ownable {
    ...
    function createRect() public view returns (string memory){
        ...
        for (uint256 i = 0; i < rectBody.length; i += 1) {
            ...
            buffer[cursor + 1] = lookup[rectBody[i][2]];        // x
            buffer[cursor + 2] = lookup[rectBody[i][3]];         // y
            buffer[cursor + 3] = colors[rectBody[i][4]];   
            cursor+=4;
            if (cursor >= 16) {
        ...
        for (uint256 i = 0; i < rectHead.length; i += 1) {
            ...
            buffer[cursor + 1] = lookup[rectHead[i][2]];        // x
            buffer[cursor + 2] = lookup[rectHead[i][3]];         // y
            buffer[cursor + 3] = colors[rectHead[i][4]];   
            cursor+=4;
            if (cursor >= 16) {
								// Write the rectangles from the buffer to a string
								// and concatenate with the existing part string. 
        ...
        for (uint256 i = 0; i < rectAccessory.length; i += 1) {
            ...
            buffer[cursor + 1] = lookup[rectAccessory[i][2]];        // x
            buffer[cursor + 2] = lookup[rectAccessory[i][3]];         // y
            buffer[cursor + 3] = colors[rectAccessory[i][4]];   
            cursor+=4;
            if (cursor >= 16) {
								// Write the rectangles from the buffer to a string
								// and concatenate with the existing part string. 
        ...
        for (uint256 i = 0; i < rectGlasses.length; i += 1) {
            ...
            buffer[cursor + 1] = lookup[rectGlasses[i][2]];        // x
            buffer[cursor + 2] = lookup[rectGlasses[i][3]];         // y
            buffer[cursor + 3] = colors[rectGlasses[i][4]];   
            cursor+=4;
            if (cursor >= 16) {
								// Write the rectangles from the buffer to a string
								// and concatenate with the existing part string. 
