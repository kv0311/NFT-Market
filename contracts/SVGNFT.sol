...
contract SVGNFT is ERC721URIStorage, Ownable {
    ...
    // [["6","1","1","11",222],["6","1","17","11",222],["1","1","1","12",222],["2","1","11","12",222],["2","1","13","12",35],["1","1","15","12",222],["1","1","17","12",222],["2","1","18","12",1],["2","1","2","12",35],["1","1","22","12",222],["4","1","7","13","19a54" ],["2","1","11","13",1],["2","1","13","13",35],["3","1","15","13",222],["2","1","18","13",1],["2","1","2","13",35],["1","1","22","13",222],["1","1","7","14","19a54" ],["1","1","1","14",222],["2","1","11","14",1],["2","1","13","14",35],["1","1","15","14",222],["1","1","17","14",222],["2","1","18","14",1],["2","1","2","14",35],["1","1","22","14",222],["1","1","7","15","19a54" ],["1","1","1","15",222],["2","1","11","15",1],["2","1","13","15",35],["1","1","15","15",222],["1","1","17","15",222],["2","1","18","15",1],["2","1","2","15",35],["1","1","22","15",222],["6","1","1","16",222],["6","1","17","16",222]]
    ...
    function createRect() public view returns (string memory){
        ...
        string memory chunk;
        string[][] memory rectBody = body[0];
        string[][] memory rectHead = head[0];
        string[][] memory rectAccessory = accessory[0];
        string[][] memory rectGlasses = glasses[0];
        ...
        for (uint256 i = 0; i < rectBody.length; i += 1) {
            chunk = string(
                abi.encodePacked(
                    chunk,
                    '<rect width="', rectBody[i][0] * 10, '" height="', rectBody[i][1], '" x="', rectBody[i][2], '" y="', rectBody[i][3], '"  fill="#', rectBody[i][4], '"/>'
                )
            );
        }
        ...
        for (uint256 i = 0; i < rectHead.length; i += 1) {
            chunk = string(
                abi.encodePacked(
                    chunk,
                    '<rect width="', rectHead[i][0], '" height="', rectHead[i][1], '" x="', rectHead[i][2], '" y="', rectHead[i][3], '"  fill="#', rectHead[i][4], '"/>'
                )
            );
        }
        for (uint256 i = 0; i < rectAccessory.length; i += 1) {
            chunk = string(
                abi.encodePacked(
                    chunk,
                    '<rect width="', rectAccessory[i][0], '" height="', rectAccessory[i][1], '" x="', rectAccessory[i][2], '" y="', rectAccessory[i][3], '"  fill="#', rectAccessory[i][4], '"/>'
                )
            );
        }
        ...
        for (uint256 i = 0; i < rectGlasses.length; i += 1) {
            chunk = string(
                abi.encodePacked(
                    chunk,
                    '<rect width="', rectGlasses[i][0], '" height="', rectGlasses[i][1], '" x="', rectGlasses[i][2], '" y="', rectGlasses[i][3], '"  fill="#', rectGlasses[i][4], '"/>'
                )
            );
        }
