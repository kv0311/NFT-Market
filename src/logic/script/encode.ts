import { PNGCollectionEncoder } from '../nouns-sdk/image';
import { readPngFile } from 'node-libpng';
import { promises as fs } from 'fs';
import path from 'path';

const DESTINATION = path.join(__dirname, '../../../../logic/data/image-data.json');
...
const encode = async () => {
  ...
  for (const folder of partfolders) {
    const folderpath = path.join(__dirname, '../../../../logic/images', folder);
